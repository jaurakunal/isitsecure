...
def fix(
    ...
    Local ``--repo`` paths get fixes applied in place on a fresh branch. A
    remote GitHub ``--repo`` URL (with ``--github-token``) is cloned and gets
    per-category pull requests opened instead.
    """
    if verbose:
        logging.basicConfig(level=logging.DEBUG)
    else:
