...
- **Mutation read-back**: for PUT/PATCH IDOR, a 2xx only proves the write was *accepted*. The probe writes a canary into one existing, non-sensitive field (never a money/auth/PII field), re-reads it, and restores the original value. Confirmed persistence → CRITICAL; accepted-but-unconfirmable (e.g. the read is auth-gated) → a downgraded HIGH lead. The SPA catch-all (200 text/html for unknown routes) is rejected before any of this, so an app-shell response is never a write.
